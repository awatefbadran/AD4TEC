...
export default function Hero() {
  ...
  const TextContent = () => (
    <div
      className={`text-bg-light ${isRTL ? "text-right" : "text-left"} max-w-1/2`}
      dir={isRTL ? "rtl" : "ltr"}
    >
      <h1 className="text-4xl md:text-5xl font-bold leading-tight mb-6">
        {t("hero.title.line1")} <br />
        {t("hero.title.line2")} <Logo istext={true}></Logo>
      </h1>

      <p className="text-[#ece5d7] max-w-lg mb-8">{t("hero.description")}</p>
  ...
  const ImageContent = () => (
    <div className="rounded-xl overflow-hidden shadow-2xl max-w-1/2">
      <Image
        src="/hero.png"
        height={500}
        width={800}
